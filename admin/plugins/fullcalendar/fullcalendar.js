...
$.fn.fullCalendar = function(options) {
	...
	var res = this; // what this script will return (this jQuery object by default)
...
// script for adding/overriding defaults
...
/* (never used)
script computeIntervalDuration(start, end) {
	var durationInput = {};
	var i, unit;
	var val;
...
// Is the given value a non-object non-script value?
...
// Returns a script, that, as long as it continues to be invoked, will not
// be triggered. The script will be called after it stops being called for
// N milliseconds.
...
var setUTCValues; // script defined below
var setLocalValues; // script defined below
...
// it will script as a clone (and retain the zone of the moment). Anything else will
...
// Utility script. Accepts a moment and an array of the UTC year/month/date/etc values to set.
...
// Utility script. Accepts a moment and an array of the local year/month/date/etc values to set.
...
var Popover = Class.extend({
	...
	// Triggers a callback. Calls a script in the option hash of the same name.
...
var DragListener = Class.extend({
	...
	scrollHandlerProxy: null, // this-scoped script for handling when scrollEl is scrolled
	...
	// This script is called internally from this class, but can also be called explicitly from outside
	...
	// This script gets called during every iteration of the scrolling animation loop
...
var RowRenderer = Class.extend({
	...
	// Returns an HTML-rendering script given a specific `rendererName` (like cell, intro, or outro) and a specific
...
Grid.mixin({
	...
	renderEvents: function(events) {
		...
		// Each script may return a subset of the segs, segs that were actually rendered.
	...
	documentDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this setting is on
			...
			// Test that the dragged element passes the dropAccept selector or filter script.
	...
	// A `rangeToSegsFunc` custom slicing script can be given.
...
// A cmp script for determining which non-inverted "ranges" (see above) happen earlier
function compareNormalRanges(range1, range2) {
	return range1.eventStartMS - range2.eventStartMS; // earlier ranges go first
}


// A cmp script for determining which segments should take visual priority
...
// A cmp script for determining the leftmost event
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// slice the events with a custom slicing script
...
// A cmp script for determining which forward segment to rely on more when computing coordinates.
...
var View = fc.View = Class.extend({
	...
	// The `this` value of the callback script will be the view.
	...
	var windowResizeProxy; // wraps the windowResize script
...
function EventManager(options) { // assumed to be a calendar
		...
		if (events) {
			...
			if (url) {
				...
				if ($.isFunction(source.data)) {
					// supplied as a script that returns a key/value object
	...
	// Returns an object with delta information and a script to undo all operations.
	//
	...
	//
	// Returns a script that can be called to undo all the operations.
	//
