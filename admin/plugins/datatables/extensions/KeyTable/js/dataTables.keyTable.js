...
KeyTable = function ( oInit )
{
	 ...
	 *           int:y - y coordinate
	 * Notes:    Thanks to Rohan Daxini for the basis of this script
	 */
	 ...
	 * Purpose:  Create a script (with closure for sKey) event addition API
	 * Returns:  script: - template script
	...
	function _fnEventAddTemplate( sKey )
	{
		/*
		 * Function: -
		 * Purpose:  API script for adding event to cache
		 * Returns:  -
		 * Inputs:   1. node:x - target node to add event for
		 *           2. script:y - callback script to apply
		 *         or
		 *           1. int:x - x coord. of target cell (can be null for live events)
		 *           2. int:y - y coord. of target cell (can be null for live events)
		 *           3. script:z - callback script to apply
		 * Notes:    This script is (interally) overloaded (in as much as javascript allows for
	 ...
	 * Purpose:  Create a script (with closure for sKey) event removal API
	 * Returns:  script: - template script
	 * Inputs:   string:sKey - type of event to detect
	 */
	function _fnEventRemoveTemplate( sKey )
	{
		/*
		 * Function: -
		 * Purpose:  API script for removing event from cache
		 ...
		 *           1. int:x - x coord. of target cell (can be null for live events)
		 *           2. int:y - y coord. of target cell (can be null for live events)
		 *           3. script:z - callback script to remove - optional
		 * Notes:    This script is (interally) overloaded (in as much as javascript allows for
		 *           that) - the target cell can be given by either node or coords and the script
	 ...
	 *           script:fn - callback script for when triggered
	 */
	 ...
	 *           script:fn - optional - remove this script. If not given all handlers of this
	 ...
	 *           but that would mean that only one script of a particular type can be
